...
export default function ProformaEmailPage() {
  ...
  React.useEffect(() => {
    ...
    setLoading(true)
    axios
      .get(`/proformas/${id}/email_draft/?company=${currentOrganizationId}`, { withCredentials: true })
  ...
  const handleSend = async (payload: EmailComposerSubmitPayload) => {
    ...
    try {
      ...
      setSuccess(null)
      await axios.post(
        `/proformas/${id}/send-email/?company=${currentOrganizationId}`,
        payload,
        { withCredentials: true }
      )
      setSuccess("Proforma email sent successfully.")
    } catch {
      setError("Failed to send proforma email.")
    } finally {
      setSending(false)
    }
  }
  ...
  return (
    <div>
      ...
      <EmailComposer
        title={`Send proforma email${draft?.proforma_number ? ` · ${draft.proforma_number}` : ""}`}
        description="The email template is prefilled from the proforma and can be edited before sending."
